...
#define IODIR 0x00 // Set the direction of I/O
#define GPIO 0x09 // use GPIO when reading from register
#define OLAT 0x0A // use OLAT when writing to register, not GPIO
#define DEV 0x40 // Device address
#define IOPIN 0xF0 // GPIO 4-7 Inputs, 0-3 outputs
...
int main() {
    ...
    // generating the sine and triangle waves to send to setVoltage
...
////////////////// SPI portion of HW 4 //////////////
///////////////// I2C - LED & PUSH BUTTON //////////
        values = getExpander(); // adjust later to include the '&' to get the specific pin
        if (values == 0x80){
            setExpander(0x01,1);
            values = 0x00;
        }
...
void i2c_expander(void){
    // port (OLAT) register sets either high or low -- set initial logics
    i2c_master_start();
    i2c_master_send(DEV);
    i2c_master_send(OLAT);
    i2c_master_send(0x00); // set all i/o as low first
    i2c_master_stop();
}
...
// set the logic of the expander
void setExpander(unsigned char pin, char level){ // working
    ...
    value_bits = (value_bits & pin); // value_bits is the command it sends, & command will get you that specific pin
    i2c_master_start();
    i2c_master_send(DEV);
    i2c_master_send(OLAT);
